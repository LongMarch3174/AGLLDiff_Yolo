...
class YOLOLoss(nn.Module):
    ...
    def bbox_mpdiou(self, box1, box2, x1y1x2y2=True, mpdiou_hw=None, grid=None, eps=1e-7):
    ...
    def __call__(self, predictions, targets, imgs):
        ...
        for i, prediction in enumerate(predictions):
            ...
            if n:
                ...
                #   计算预测框和真实框的回归损失
                # -------------------------------------------#
                iou = self.bbox_mpdiou(box.T, selected_tbox, x1y1x2y2=False, mpdiou_hw=prediction.size(2) ** 2 + prediction.size(3) ** 2, grid=torch.stack([gj, gi]))

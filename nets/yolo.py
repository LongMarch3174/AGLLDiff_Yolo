...
from nets.gold_yolo.reppan import RepGDNeck
from nets.gold_yolo.gold_config import RepGDNeck_config
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, pretrained=False):
        ...
        self.neck_config = RepGDNeck_config()
        self.neck = RepGDNeck(channels_list=self.neck_config.channels_list, num_repeats=self.neck_config.num_repeats, extra_cfg=self.neck_config.extra_cfg)
        # ------------------------加强特征提取网络------------------------#

        # 80, 80, 64 => 80, 80, 256
        self.rep_conv_1 = conv(transition_channels * 2, transition_channels * 8, 3, 1)
        # 40, 40, 128 => 40, 40, 512
        self.rep_conv_2 = conv(transition_channels * 4, transition_channels * 16, 3, 1)
        # 20, 20, 256 => 20, 20, 1024
        self.rep_conv_3 = conv(transition_channels * 8, transition_channels * 32, 3, 1)
    ...
    def forward(self, x):
        ...
        # ------------------------加强特征提取网络------------------------#
        # 20, 20, 1024 => 20, 20, 512
        B5 = self.sppcspc(feat3)
        # 40, 40, 1024 => 40, 40, 256
        B4 = self.conv_for_feat2(feat2)
        # 80, 80, 512 => 80, 80, 128
        B3 = self.conv_for_feat1(feat1)
        # 160, 160, 256 => 160, 160, 64
        B2 = self.conv_for_feat0(feat0)

        gold_input = (B2, B3, B4, B5)
        gold_output = self.neck(gold_input)
        ...
        P3 = self.rep_conv_1(gold_output[0])
        P4 = self.rep_conv_2(gold_output[1])
        P5 = self.rep_conv_3(gold_output[2])
        # -------------------------P3--------------------------#
        #   第三个特征层

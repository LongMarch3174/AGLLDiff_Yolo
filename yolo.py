...
class YOLO(object):
    _defaults = {
        ...
        # --------------------------------------------------------------------------#
        "model_path": resource_path('yolo_weights/best_epoch_weights.pth'),
    ...
    def detect_image(self, image, crop=False, count=False):
        """
        :param image: PIL.Image 输入
        :param crop: 是否裁剪目标
        :param count: 是否统计各类别数量
        :return: 如果 count=False，返回 PIL.Image；
                 如果 count=True，返回 (PIL.Image, {class_name: count})
        """
        # 原有前置处理
        ...
        counts_dict = {}
        if count:
            for cls_idx in np.unique(top_label):
                cnt = int((top_label == cls_idx).sum())
                if cnt > 0:
                    cls_name = self.class_names[cls_idx]
                    counts_dict[cls_name] = cnt
        ...
        if crop:
            for i, box in enumerate(top_boxes):
                ...
                crop_img.save(os.path.join(crop_dir, f"crop_{i}.png"), quality=95, subsampling=0)
        ...
        for i, cls_idx in enumerate(top_label):
            ...
            for t in range(thickness):
                draw.rectangle(
                    ...
                    outline=self.colors[cls_idx]
                )
            # 绘制标签背景
            text_origin = (left, top - label_size[1] if top - label_size[1] >= 0 else top + 1)
            draw.rectangle(
                [text_origin, (text_origin[0] + label_size[0], text_origin[1] + label_size[1])],
                fill=self.colors[cls_idx]
            )
            draw.text(text_origin, label, fill=(0, 0, 0), font=font)

        # 根据 count 标志返回
        if count:
            return image, counts_dict
        return image
    ...
    def filter_detect(self, image):
        """
        检测输入的 PIL.Image，并返回检测结果列表。

        参数:
            image: PIL.Image 对象，输入检测图像
            class_names: list, 类别名称列表

        返回:
            detections: list of tuples，每个元组格式为
                        (类别名称, 检测得分, x1, y1, x2, y2)
        """
        # 计算输入图片的高宽
        image_shape = np.array(np.shape(image)[0:2])
        # 转换为RGB图像（防止灰度图出错）
        image = cvtColor(image)
        # 处理图像尺寸：letterbox resize 保持长宽比例（或直接 resize）
        image_data = resize_image(image, (self.input_shape[1], self.input_shape[0]), self.letterbox_image)
        # 转换为 float32，并调整通道顺序和 batch_size
        image_data = np.expand_dims(np.transpose(preprocess_input(np.array(image_data, dtype='float32')), (2, 0, 1)), 0)
        ...
        detections = []
        # image.size 中：image.size[0] 为宽, image.size[1] 为高
        for i, c in enumerate(top_label):
            predicted_class = self.class_names[int(c)]
            score = float(top_conf[i])
            # 注意：top_boxes[i] 中顺序为：top, left, bottom, right
            top, left, bottom, right = top_boxes[i]
            top = max(0, int(np.floor(top)))
            left = max(0, int(np.floor(left)))
            bottom = min(image.size[1], int(np.floor(bottom)))
            right = min(image.size[0], int(np.floor(right)))
            detection = (predicted_class, score, left, top, right, bottom)
...
class YOLO_ONNX(object):
    _defaults = {
        ...
        #   anchors_mask用于帮助代码找到对应的先验框，一般不修改。
        # ---------------------------------------------------------------------#
        "anchors_path": 'model_data/yolo_anchors_v0.1.txt',
        "anchors_mask": [[6, 7, 8], [3, 4, 5], [0, 1, 2]],
        # ---------------------------------------------------------------------#
        #   输入图片的大小，必须为32的倍数。
        # ---------------------------------------------------------------------#
